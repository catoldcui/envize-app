        ...
        {profiles.map((p) => {
          ...
          return (
            <div key={p.name} className={`flex items-center justify-between px-3 py-2 cursor-pointer ${isSelected ? "bg-blue-100" : "hover:bg-gray-100"}`} onClick={() => onSelectProfile(p.name)}>
              ...
              <button role="switch" aria-checked={isActive} onClick={(e) => { e.stopPropagation(); onToggleProfile(p.name, !isActive); }} className={`w-8 h-4 rounded-full relative transition-colors ${isActive ? "bg-green-500" : "bg-gray-300"}`}>
                <span className={`absolute top-0.5 w-3 h-3 rounded-full bg-white transition-transform ${isActive ? "translate-x-4" : "translate-x-0.5"}`} />
              </button>
            </div>
          );
        })}
